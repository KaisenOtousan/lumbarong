  ...
  (config) => {
    try {
      if (typeof window !== "undefined") {
        ...
        } else if (path.startsWith("/home") || path.startsWith("/profile") || path.startsWith("/checkout") || path.startsWith("/messages")) {
          token = localStorage.getItem("customer_token") || localStorage.getItem("token");
        } else {
          // Absolute fallback: try role-specific keys if others aren't found
